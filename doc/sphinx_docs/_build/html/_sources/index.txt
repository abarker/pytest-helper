...
These are the main functions which are used to make it easier to invoke pytest
on a test file.
...
* :ref:`pytest_helper.script_run<script_run>`
   ...
   would be used to run pytest, with the ``-v`` verbose argument, on the test
   file ``test/test_foobar.py``.::
...
* :ref:`pytest_helper.clear_locals_from_globals<clear_locals_from_globals>`
   ...
   The `clear_locals_from_globals` function is called by `locals_to_globals` to
   clear any previously-set globals.  It can also be used to explicitly do the
   clearing, such as when `locals_to_globals` is run with `auto_clear` set false.
...
* **Using pytest-helper inside a separate test file.**
   ...
   `script_run` or via the usual invocation of pytest from the command line. ::
      ...
      # pytest_helper.sys_path("..")  # Works the same as the line above.
      # pytest_helper.sys_path([".."])  # Works the same as the two lines above.
...
then the actual import of the code is done.  If desired, the original module
can use `script_run` to run pytest on the new test file when it is invoked as a
script, as in the earlier example.
